...
const handleSignIn = async (e: React.FormEvent) => {
  ...
  try {
    ...
    // ✔ Si token → on l’enregistre
    if (data?.token?.trim()) {
      localStorage.setItem("authToken", data.token.trim());
    }

    // ✔ Même sans token → login accepté (Render)
  ...
  <div className="min-h-screen bg-background flex items-center justify-center p-4">
    <Card className="w-full max-w-md shadow-xl border-0 card transition-opacity duration-700">
      ...
      <CardContent>
        ...
        {alert && (
          <div
            className={`flex items-center gap-2 mb-4 p-3 rounded-md ${alert.type === "success"
